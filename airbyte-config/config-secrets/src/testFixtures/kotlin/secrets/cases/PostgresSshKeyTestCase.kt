...
class PostgresSshKeyTestCase : SecretsTestCase {
  override val name: String
    get() = "postgres_ssh_key"
  ...
  override val persistenceUpdater: Consumer<SecretPersistence>
    get() {
      return Consumer<SecretPersistence> { secretPersistence: SecretPersistence ->
        secretPersistence.write(AirbyteManagedSecretCoordinate(SecretsTestCase.buildBaseCoordinate(uuidIndex = 0), 1), "hunter1")
        secretPersistence.write(AirbyteManagedSecretCoordinate(SecretsTestCase.buildBaseCoordinate(uuidIndex = 1), 1), "hunter2")
      }
    }
}
