...
internal class ConnectorBuilderControllerHttpClientTest {
  ...
  @MockBean(FullResolveManifestHandler::class)
  fun fullResolveManifestHandler(): FullResolveManifestHandler = mockk()

  @MockBean(ResolveManifestHandler::class)
  fun resolveManifestHandler(): ResolveManifestHandler = mockk()

  @MockBean(AirbyteCdkRequester::class)
  fun airbyteCdkRequester(): AirbyteCdkRequester = mockk()
  ...
  fun `verify health endpoint`() {
    val request = HttpRequest.GET<Any>("/api/v1/connector_builder/health")
  ...
  fun `verify missing endpoint`() {
    val request = HttpRequest.GET<Any>("/api/v1/connector_builder/health/missing")
