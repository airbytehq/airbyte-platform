...
class AirbyteCloudStorageAppender(
  ...
  private val uniqueIdentifier = UUID.randomUUID().toString().replace("-", "")
  private var currentStorageId: String = composeId()
  ...
  override fun stop() {
    try {
      ...
      // Do one final upload attempt to make sure all logs are published
      upload()
    }
  }
  ...
  private fun upload() {
    ...
    if (messages.isNotEmpty()) {
      ...
      this.currentStorageId = composeId()
    }
  }

  private fun composeId(): String {
    // Remove the trailing "/" from the base storage ID if present to avoid duplicates in the storage ID
    return "${baseStorageId.trimEnd('/')}/${LocalDateTime.now().format(DATE_FORMAT)}_${hostname}_$uniqueIdentifier"
  }
}
