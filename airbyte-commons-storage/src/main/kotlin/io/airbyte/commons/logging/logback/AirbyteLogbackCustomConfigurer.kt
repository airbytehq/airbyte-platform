...
class AirbyteLogbackCustomConfigurer :
  ...
  internal fun createApplicationRollingAppender(
    ...
    appender.name = "$discriminatorValue-local"
    appender.encoder = createEncoder(context = context, layout = AirbytePlatformLogbackMessageLayout())
    appender.file = baseFile

    val triggeringPolicy = SizeBasedTriggeringPolicy<ILoggingEvent>()
    triggeringPolicy.context = context
    triggeringPolicy.maxFileSize = FileSize.valueOf(DEFAULT_MAX_LOG_FILE_SIZE)
    triggeringPolicy.start()

    val rollingPolicy = FixedWindowRollingPolicy()
    rollingPolicy.context = context
    rollingPolicy.fileNamePattern = baseFile.replace(LOG_FILE_EXTENSION, ROLLING_FILE_NAME_PATTERN)
  ...
  internal fun createOperationsJobFileAppender(
    ...
    appender.context = context
    appender.encoder = createEncoder(context = context, layout = AirbyteOperationsJobLogbackMessageLayout())
    appender.file = filePath.toString()
    appender.name = "$discriminatorValue-local"
    appender.start()
    return appender
  }
  ...
  private fun registerShutdownHook(loggerContext: LoggerContext) {
    ...
    Runtime.getRuntime().addShutdownHook(Thread { shutdownHook.run() })
  }
}
