...
private class AirbytePlatformLogbackMessageLayoutTest {
  ...
  fun testCiModeLogMessage() {
    ...
    expected.append("1970-01-01 00:00:00,000 [dd.trace_id=$traceId dd.span_id=$spanId] ")
    expected.append("[$logThreadName]$TAB$ESC_START${ANSIConstants.BLUE_FG}$ESC_END${logLevel}$DEFAULT_COLOR$TAB")
    expected.append("${formatClassName(className)}($methodName):$lineNumber $DASH_CHAR ")
    expected.append("$logMessage$LINE_SEPARATOR")
  ...
  fun tesLogMessage() {
    ...
    val expected = StringBuilder()
    expected.append("1970-01-01 00:00:00,000 ")
    expected.append("[$logThreadName]$TAB$ESC_START${ANSIConstants.BLUE_FG}$ESC_END${logLevel}$DEFAULT_COLOR$TAB")
    expected.append("${formatClassName(className)}($methodName):$lineNumber $DASH_CHAR ")
    expected.append("$logMessage$LINE_SEPARATOR")
    assertEquals(expected.toString(), message)
  ...
  fun tesLogMessageWithMaskedData() {
    ...
    expected.append("[$logThreadName]$TAB$ESC_START${ANSIConstants.BLUE_FG}$ESC_END${logLevel}$DEFAULT_COLOR$TAB")
    expected.append("${formatClassName(className)}($methodName):$lineNumber $DASH_CHAR ")
    expected.append("${logMessage.replace(apiKey, SECRETS_MASK)}$LINE_SEPARATOR")
    assertEquals(expected.toString(), message)
  }
  ...
  fun tesLogMessageWithException() {
    ...
    val expected = StringBuilder()
    expected.append("1970-01-01 00:00:00,000 ")
    expected.append("[$logThreadName]$TAB$ESC_START${ANSIConstants.BOLD + ANSIConstants.RED_FG}$ESC_END${logLevel}$DEFAULT_COLOR$TAB")
    expected.append("${formatClassName(className)}($methodName):$lineNumber $DASH_CHAR ")
    expected.append(logMessage)
    expected.append("$LINE_SEPARATOR${throwableConverter.convert(event)}")
  ...
  fun tesLogMessageWithCallerContext() {
    ...
    val expected = StringBuilder()
    expected.append("1970-01-01 00:00:00,000 ")
    expected.append("[$callerThreadName]$TAB$ESC_START${ANSIConstants.BLUE_FG}$ESC_END${logLevel}$DEFAULT_COLOR$TAB")
    expected.append("${formatClassName(callerClassName)}($callerMethodName):$callerLineNumber $DASH_CHAR ")
    expected.append("$logMessage$LINE_SEPARATOR")
    assertEquals(expected.toString(), message)
  }
}
