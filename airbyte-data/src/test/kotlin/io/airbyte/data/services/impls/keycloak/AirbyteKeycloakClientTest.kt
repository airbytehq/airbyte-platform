...
class AirbyteKeycloakClientTest {
  ...
  @Test
  fun `exchangeAuthorizationCode successfully exchanges code for token`() {
    ...
    val tokenEndpoint = "http://localhost/realms/$realm/protocol/openid-connect/token"

    every { keycloakConfiguration.getKeycloakTokenEndpointForRealm(realm) } returns tokenEndpoint

    val mockCall = mockk<Call>()
    val mockResponse =
      okhttp3.Response
        .Builder()
        .request(Request.Builder().url(tokenEndpoint).build())
        .protocol(Protocol.HTTP_1_1)
        .code(200)
        .message("OK")
        .body("{\"access_token\":\"test-access-token\",\"token_type\":\"Bearer\"}".toResponseBody())
        .build()
    ...
    assertTrue(accessToken == "test-access-token")
  }

  @Test
  fun `exchangeAuthorizationCode throws InvalidTokenException for 400 response`() {
    val realm = "test-realm"
    ...
    val mockResponse =
      okhttp3.Response
        ...
        .code(400)
        .message("Bad Request")
        .body("{\"error\":\"invalid_grant\",\"error_description\":\"Code not valid\"}".toResponseBody())
        .build()
  ...
  @Test
  fun `exchangeAuthorizationCode throws MalformedTokenResponseException for empty response`() {
    val realm = "test-realm"
    val authCode = "auth-code-123"
    val codeVerifier = "verifier-xyz"
    val redirectUri = "https://cloud.airbyte.com/callback"
  ...
  @Test
  fun `getUserInfo successfully returns user info map`() {
    val token = "test-access-token"
    val realm = "test-realm"
    val userInfoEndpoint = "http://localhost/realms/$realm/protocol/openid-connect/userinfo"

    every { keycloakConfiguration.getKeycloakUserInfoEndpointForRealm(realm) } returns userInfoEndpoint
    ...
    val mockResponse =
      okhttp3.Response
        ...
        .code(200)
        .message("OK")
        .body("{\"sub\":\"user-123\",\"email\":\"test@airbyte.com\",\"email_verified\":true}".toResponseBody())
        .build()

    every { mockHttpClient.newCall(any()) } returns mockCall
    every { mockCall.execute() } returns mockResponse

    val userInfo = airbyteKeycloakClient.getUserInfo(token, realm)

    assertTrue(userInfo["email"] == "test@airbyte.com")
    assertTrue(userInfo["sub"] == "user-123")
    assertTrue(userInfo["email_verified"] == true)
  }

  @Test
  fun `getUserInfo throws TokenExpiredException for 401 response`() {
    ...
    assertThrows<TokenExpiredException> {
      airbyteKeycloakClient.getUserInfo(token, realm)
    }
  }

  @Test
  fun `getUserInfo throws InvalidTokenException for non-200 non-401 response`() {
    val token = "test-access-token"
    val realm = "test-realm"
    val userInfoEndpoint = "http://localhost/realms/$realm/protocol/openid-connect/userinfo"
