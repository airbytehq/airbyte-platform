...
private val SSO_REALM = "test-realm"
...
class OrganizationServiceDataImplTest {
  ...
  @BeforeEach
  fun setUp() {
    organizationServiceDataImpl = OrganizationServiceDataImpl(organizationRepository, ssoConfigRepository)
  }
  ...
  fun `listOrganizationsByUserId returns organizations for user`() {
    ...
    val organizations = listOf(ENTITY_ORGANIZATION)

    every { organizationRepository.findByUserId(USER_ID, "test", includeDeleted) } returns organizations
    every { ssoConfigRepository.findByOrganizationIdIn(listOf(ORGANIZATION_ID)) } returns listOf(SSO_CONFIG)
  ...
  fun `listOrganizationsByUserId returns empty list when no organizations found`() {
    ...
    every { organizationRepository.findByUserId(USER_ID, null, includeDeleted) } returns emptyList()
  ...
  fun `listOrganizationsByUserIdPaginated returns paginated organizations for user`() {
    ...
    every { organizationRepository.findByUserIdPaginated(USER_ID, "test", false, 10, 0) } returns organizations
    every { ssoConfigRepository.findByOrganizationIdIn(listOf(ORGANIZATION_ID)) } returns listOf(SSO_CONFIG)
  ...
  fun `listOrganizationsByUserIdPaginated returns empty list when no organizations found`() {
    ...
    every { organizationRepository.findByUserIdPaginated(USER_ID, null, true, 5, 10) } returns emptyList()
