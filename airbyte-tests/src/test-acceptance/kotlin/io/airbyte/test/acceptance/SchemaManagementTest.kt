...
internal class SchemaManagementTest {
  ...
  private fun createTestConnections() {
    ...
    createdConnection =
      atClient.admin
        .getConnection(conId)
        .also { log.info { "Created connection: $it" } }
    ...
    createdConnectionWithSameSource =
      atClient.admin
        ...
        .also { log.info { "Created connection (with same source): $it" } }
  }
  ...
  fun `backfill disabled`() {
    ...
    val statusWithBackfills = atClient.admin.jobWatchUntilTerminal(jobIdWithBackfills)
  ...
  fun `apply empty schema change`() {
    ...
    assertEquals(postUpdate.syncCatalog, postUpdate.syncCatalog)
