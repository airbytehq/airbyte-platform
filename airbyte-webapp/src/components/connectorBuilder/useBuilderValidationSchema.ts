...
export const useBuilderValidationSchema = () => {
  ...
  function validatePartitionRouterTypes(
  ...
  function validateListPartitionRouterValues(
  ...
  const builderFormValidationSchema = yup.object().shape({
    global: globalSchema,
    streams: yup.array().min(1).of(streamSchema),
  });

  const builderStateValidationSchema = yup.object().shape({
