...
import { flattenSyncSchemaFields, getFieldPathDisplayName } from "../utils";
...
export function calculateSharedFields(selectedBatchNodes: SyncSchemaStream[]) {
  ...
  primitiveFieldsByStream.forEach((fields, index) => {
    if (index === 0) {
      fields.forEach((field) => pathMap.set(getFieldPathDisplayName(field.path), field));
    } else {
      const fieldMap = new Set(fields.map((f) => getFieldPathDisplayName(f.path)));
