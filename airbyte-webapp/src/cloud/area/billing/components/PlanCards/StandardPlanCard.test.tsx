...
describe("StandardPlanCard", () => {
  it("renders the Subscribe label and uses the setup flow by default", async () => {
    ...
    expect(useRedirectToCustomerPortal).toHaveBeenCalledWith("setup");
  ...
  it("renders the Downgrade label and uses the portal flow in downgrade mode", async () => {
    await render(<StandardPlanCard disabled={false} mode="downgrade" />);

    expect(useRedirectToCustomerPortal).toHaveBeenCalledWith("portal");
