...
export const validateAndMapEvent = (event: ConnectionEvent | ConnectionTimelineRunningEvent) => {
  ...
  const hasOnlyResourceRequirementPatches = (summary: unknown): boolean => {
    if (!summary || typeof summary !== "object" || !("patches" in summary)) {
      return true;
    }
    const { patches } = summary as { patches?: Record<string, unknown> };
    return !patches || (Object.keys(patches).length === 1 && "resourceRequirements" in patches);
  };
