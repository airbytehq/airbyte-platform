...
import { useCurrentOrganizationIdWithoutUser } from "area/organization/utils";
...
const LDInitializationWrapper: React.FC<React.PropsWithChildren<{ apiKey: string }>> = ({ children, apiKey }) => {
  ...
  const organizationId = useCurrentOrganizationIdWithoutUser();
  ...
  const [contextState, dispatchContextUpdate] = useReducer(contextReducer, {
    context: createMultiContext(
      ...
      ...(organizationId ? [createLDContext("organization", organizationId)] : [])
    ),
  });
