# Contributing to airbyte-webapp

For the project overview and setup basics, see [README.md](./README.md). Agent-specific
...
- Use `pnpm@8.6.12` and Node 20.19.0, as pinned by the project. Run commands from
  `oss/airbyte-webapp/`.
- All user-facing text uses React Intl. Add keys to `src/locales/en.json` (or
  `en.errors.json` for error messages), and remove unused keys when removing code.
- Do not edit or commit generated API artifacts under `src/core/api/generated/`.
  If the generated shape is wrong, fix the OpenAPI source and regenerate the client.
- Custom hooks belong in `src/core/api/hooks/`, and tests should be co-located with
  the component or module they cover.
- SCSS modules are still in use; follow existing styling patterns rather than
  introducing Tailwind.

## Verification

Run the following commands from `oss/airbyte-webapp/` when verifying a webapp change:
...
Use `pnpm test:ci` for non-watch test runs. The `pnpm test` command is interactive
and does not exit on its own.
...
## Mocking APIs and features in development

Mocks and fake data must never ship as a runtime surface in production bundles. When you need to mock an API or feature state during development:

- Use dev-only code paths that are dead-code-eliminated from production builds. The pattern to follow is `ExperimentService` (`src/core/services/Experiment/ExperimentService.tsx`): overrides load from a local `.experiments.dev` file and apply only when `process.env.NODE_ENV === "development"`.
- For end-to-end tests, use the dedicated e2e hooks (e.g. `window._e2eOverwrites`) rather than inventing new override mechanisms.
- Any mocking library or tooling belongs in `devDependencies`, never in `dependencies`.
- Do not introduce bespoke runtime toggles (e.g. localStorage keys read in production code) to switch mock behavior on and off.

For the canonical repository-wide contribution rules, see the [root CONTRIBUTING.md](../../CONTRIBUTING.md).
