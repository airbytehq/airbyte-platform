...
class AppendToAttemptLogActivityTest {
  ...
  void setsMdc(final Path path, final WorkerEnvironment env, final long jobId, final int attemptNumber) {
    final var activity = new AppendToAttemptLogActivityImpl(mLogger, mLogClientSingleton, mLogConfigs, path, env);
  ...
  void logsMsgInfo(final String msg) {
    final var activity = new AppendToAttemptLogActivityImpl(mLogger, mLogClientSingleton, mLogConfigs, Fixtures.path1, Fixtures.docker);
