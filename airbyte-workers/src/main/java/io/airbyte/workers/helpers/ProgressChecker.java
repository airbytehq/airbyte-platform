...
public class ProgressChecker {
  ...
  public boolean check(final long jobId, final int attemptNo) {
    ...
    final var resp = AirbyteApiClient.retryWithJitter(
        () -> attemptApi.getAttemptCombinedStats(req),
        String.format("Checking progress stats for job: %d attempt: %d", req.getJobId(), req.getAttemptNumber()));

    if (resp == null) {
      recordCheckFailure(jobId, attemptNo);
      log.info(String.format("Unable to check progress stats for job: %d attempt: %d", req.getJobId(), req.getAttemptNumber()));
    }
