...
class WorkloadClient(
  ...
  fun createWorkload(workloadCreateRequest: WorkloadCreateRequest) {
    try {
      val response = workloadApiClient.workloadApi.workloadCreate(workloadCreateRequest).execute()
      if (!response.isSuccessful) {
        if (response.code() == HttpStatus.CONFLICT.code) {
  ...
  fun isTerminal(workloadId: String) =
    isWorkloadTerminal(
      workloadApiClient.workloadApi
        .workloadGet(workloadId)
        .body(),
    )
  ...
  fun waitForWorkload(
    ...
    try {
      ...
      while (!isWorkloadTerminal(workload)) {
        ...
        workload = workloadApiClient.workloadApi.workloadGet(workloadId).body()
      }
    } catch (e: IOException) {
      throw RuntimeException(e)
  ...
  private fun handleMissingConnectorJobOutput(
    ...
    return Result
      .runCatching {
        val workload = workloadApiClient.workloadApi.workloadGet(workloadId).body()
