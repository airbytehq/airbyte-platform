...
/**
 * Tests that using `scopes` array produces the exact same consent URL as using `scope` string.
 * This proves backward compatibility: connectors can migrate from scope to scopes without
 * changing their URL templates.
 */
internal class DeclarativeOAuthFlowScopesArrayTest : BaseOAuthFlowTest() {
  override val oAuthFlow: BaseOAuthFlow
    get() {
      val oauthFlow =
        DeclarativeOAuthFlow(httpClient) { this.constantState }
    ...
    get() =
      Jsons.jsonNode(
        mapOf(
          ...
          "scopes" to
            listOf(
              mapOf("scope" to "test_scope_1"),
              mapOf("scope" to "test_scope_2"),
              mapOf("scope" to "test_scope_3"),
            ),
          "access_token_url" to "https://some.domain.com/oauth2/token/",
          "access_token_headers" to Jsons.jsonNode(mapOf("test_header" to "test_value")),
          "state_key" to "my_state_key",
  ...
  companion object {
    ...
    private val ARRAY_TYPE: JsonNode =
      JsonNodeFactory.instance
        ...
        .set("items", STRING_TYPE)
    private val OBJECT_TYPE: JsonNode = JsonNodeFactory.instance.objectNode().put("type", "object")
    private val OBJECT_ARRAY_TYPE: JsonNode =
      JsonNodeFactory.instance
        .objectNode()
        .put("type", "array")
        .set("items", OBJECT_TYPE)
  }
}
