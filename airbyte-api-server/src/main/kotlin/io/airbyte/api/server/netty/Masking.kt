...
private const val stringFieldMatchRegex = "(\"%s\": *)(\".*?[^\\\\]\")( *[, \\n\\r}]?)"

private val GENERIC_MASKING_TERMS = listOf(
...
private val AWS_MASKING_TERMS = listOf(
...
private val FIELDS_TO_MASK = Stream.of(
...
val headerStringMasks = HEADERS_TO_MASK.associate { header -> header.lowercase() to defaultMask }.toMap()
...
fun maskBody(
  ...
  for ((key, value) in fieldStringMasks) {
    maskedBody = Regex(java.lang.String.format(stringFieldMatchRegex, key)).replace(maskedBody) {
