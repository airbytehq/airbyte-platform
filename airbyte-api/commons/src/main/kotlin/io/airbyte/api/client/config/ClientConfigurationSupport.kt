...
fun getUrlTags(httpUrl: HttpUrl?): Array<MetricAttribute> {
  return httpUrl?.let {
    ...
    if (last.contains(UUID_REGEX)) {
      return arrayOf(MetricAttribute("url", httpUrl.toString().removeSuffix(last)), MetricAttribute("workload-id", last))
    } else {
      return arrayOf(MetricAttribute("url", httpUrl.toString()))
    }
  } ?: emptyArray()
}
