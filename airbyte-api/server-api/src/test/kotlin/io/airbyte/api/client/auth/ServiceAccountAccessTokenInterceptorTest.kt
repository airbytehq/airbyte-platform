...
internal class ServiceAccountAccessTokenInterceptorTest {
  @Test
  fun `fetches new token if cached token is expired`() {
    ...
    val interceptor =
      ServiceAccountAccessTokenInterceptor(
        serviceAccountId = serviceAccountId.toString(),
        serviceAccountSecret = "test-client-secret",
        serviceAccountTokenEndpoint = "http://example.com/token",
        httpClient = httpClient,
      )

    interceptor.cachedToken =
      ServiceAccountAccessTokenInterceptor.CachedToken(
        token = "expiredtoken",
  ...
  fun `test that when token fetching fails, the original request is proceeded`() {
    ...
    val failingInterceptor =
      object : ServiceAccountAccessTokenInterceptor(
        serviceAccountId = UUID.randomUUID().toString(),
        serviceAccountSecret = "test-client-secret",
        serviceAccountTokenEndpoint = "http://example.com/token",
