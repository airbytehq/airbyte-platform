...
open class RoleResolver(
  ...
  fun resolveRoles(
    ...
    if (authUserIds.size == 1 && authUserIds.first() == currentAuthUserId) {
      roles.add(AuthRoleConstants.SELF)
    }

    if (perms.any { it.permissionType == PermissionType.INSTANCE_ADMIN }) {
      roles.addAll(impliedRoles(PermissionType.INSTANCE_ADMIN))
    }
...
private fun determineWorkspaceRole(
  ...
  if (permOrgIds != workspaceIds) {
    return null
  }
  return workspacePerms.minBy { it.sortKey() }.permissionType
}
...
private fun determineOrganizationRole(
  ...
  if (permOrgIds != organizationIds) {
    return null
  }
  return orgPerms.minBy { it.sortKey() }.permissionType
}

private fun Permission.sortKey() =
  Enums
    .convertTo(this.permissionType, WorkspaceAuthRole::class.java)
    ?.authority
    ?: Integer.MAX_VALUE
