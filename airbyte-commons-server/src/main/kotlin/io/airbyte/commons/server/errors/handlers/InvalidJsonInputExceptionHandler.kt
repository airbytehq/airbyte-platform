...
class InvalidJsonInputExceptionHandler : ExceptionHandler<JsonMappingException, HttpResponse<*>> {
  override fun handle(
    ...
    log.error {
      "Invalid json input exception [errorId: $errorId]: ${KnownException.infoFromThrowableWithMessageAndStackTrace(
        exception,
        "Invalid json input. ${exception.message} ${exception.originalMessage}",
      )}"
    }
