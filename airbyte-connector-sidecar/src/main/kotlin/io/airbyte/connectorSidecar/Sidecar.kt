...
class Sidecar(
    ...
    mainThread =
      thread {
        ...
        try {
          ...
          while (!Files.exists(outputPath) && !Files.exists(Path.of(configDir, EXIT_CODE_FILE))) {
            Thread.sleep(100)
          }
          ...
          val streamFactory: AirbyteStreamFactory =
          ...
          val connectorOutput = worker.runCheck(outputIS, streamFactory, connectionConfiguration, exitCode)
  ...
  private fun failWorkload(
    ...
    if (failureReason != null) {
      ...
      workloadApi.workloadFailure(WorkloadFailureRequest(workloadId, null, null))
    }
  }
}
