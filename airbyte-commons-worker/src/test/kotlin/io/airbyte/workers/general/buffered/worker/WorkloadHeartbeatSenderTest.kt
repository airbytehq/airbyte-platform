...
class WorkloadHeartbeatSenderTest {
  ...
  fun `test normal flow - single successful heartbeat then GONE`() =
    runTest {
      ...
      every { mockWorkloadApiClient.workloadApi.workloadHeartbeat(any()) } throws ClientException("Gone", HttpStatus.GONE.code)
  ...
  fun `test skip heartbeat if source not beating, then GONE second iteration`() =
    runTest {
      ...
      every { mockSourceTimeoutMonitor.timeSinceLastBeat } returnsMany listOf(Optional.of(Duration.ZERO), Optional.of(Duration.ZERO))
