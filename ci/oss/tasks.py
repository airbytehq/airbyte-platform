...
async def build_oss_backend_task(settings: OssSettings, ctx: PipelineContext, client: Client, scan) -> Container:
    ...
    gradle_command = ["./gradlew", "assemble", "-x", "buildDockerImage", "-x", "dockerBuildImage", "publishtoMavenLocal", "--build-cache", "--no-daemon", "-DciMode=true"]
    ...
    result = (with_gradle(client, ctx, settings, sources_to_include=files_from_host, directory=base_dir)
                ...
                .with_exec(["./gradlew", ":airbyte-config:specs:downloadConnectorRegistry", "--rerun", "--build-cache", "--no-daemon", "-DciMode=true"])
...
async def check_oss_backend_task(client: Client, oss_build_result: Container, settings: OssSettings, ctx: PipelineContext, scan: bool) -> Container:
    gradle_command = ["./gradlew", "check", "-x", "test", "-x", ":airbyte-webapp:check","-x", "buildDockerImage", "-x", "dockerBuildImage", "--build-cache", "--no-daemon", "-DciMode=true"]
    files_from_result = [
                        "**/build.gradle", 
                        "**/gradle.properties", 
                        "**/settings.gradle",
